angular.module('brushfire').controller('editProfilePageController', ['$location', '$routeParams', '$scope', '$http', 'toastr', function($location, $routeParams, $scope, $http, toastr) {
  ...
  $http.get(theRoute)
    ...
    .catch(function onError(sailsResponse) {
      ...
      if (sailsResponse.status === 404) {
        ...
        $scope.editProfile.errorMsg = `An unexpected error occurred: ${JSON.stringify(
  ...
  $scope.changeMyPassword = function() {
    $http.put('/user/changePassword', {
      ...
      .catch(function onError(sailsResponse) {
        $scope.editProfile.changePassword.errorMsg = `An unexpected error occurred: ${JSON.stringify(
          sailsResponse.data || sailsResponse.status)}`;
      })
      .finally(function eitherWay() {
        $scope.editProfile.loading = false;
